## Monitoring
本MonitoringプロジェクトはRAGプロジェクトの上監視操作を追加されています。
...
※本日プロジェクトをビルドする時は三つ（Grafana、PrometheusとQdrant）はDockerでホストされています。最初に一般でビルドする時はビルド失敗する、可能性があります。
一つまたは二つだけもビルドを出来て、失敗ちゃった場合、も一度ビルドしてもDockerはスキップします（理由は一つだけビルドを出来たので、既に存在していると思ってしまいます）。
そういうことが行った場合、Dockerに開いて、以前の全部コンテナイメージを削除して、ビルドしてください。
